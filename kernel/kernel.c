...
void term_putchar(char c) {
    if (c == '\n') { col = 0; if (++row >= ROWS) scroll(); }
        ...
        if (col > 0) { col--; VGA[row*COLS+col] = (uint16_t)((uint16_t)attr << 8 | ' '); }
    } else {
        VGA[row*COLS+col] = (uint16_t)((uint16_t)attr << 8 | (unsigned char)c);
...
void kpanic(const char *msg) {
    uint8_t saved = attr;
    term_set_color(VGA_RED, VGA_WHITE); term_puts(" KERNEL PANIC ");
    term_set_color(VGA_YELLOW, VGA_RED); term_putchar(' '); term_puts(msg); term_putchar('\n');
...
static void banner_row(const char *s, uint8_t fg, uint8_t bg) {
    ...
    term_putchar(' ');
    const char *p = s; int len = 0; while (*p++) len++;
    term_puts(s);
...
void draw_banner(void) {
    ...
    banner_row("x86-32 ver. 1.5.2 20260627", VGA_LIGHT_GREY, VGA_BLUE);
    banner_row("", VGA_BLACK, VGA_CYAN);
...
void kernel_main(uint8_t boot_drive) {
    ...
    if (!fat_ready()) {
        ...
        char h = "0123456789ABCDEF"[boot_drive >> 4]; term_putchar(h);
        h = "0123456789ABCDEF"[boot_drive & 0xF]; term_putchar(h);
