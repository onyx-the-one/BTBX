...
/* ── FAT12 / 1.44 MB floppy geometry ──────────────────────────────────── */
#define SPT       18
#define HEADS      2
#define SECTOR_SZ 512

/* Layout — must match mkfat.py */
#define FAT1_LBA   5
#define FAT_SECS   9
...
/*
 * DMA bounce buffer — fixed in low memory at 0x0800 (512 bytes).
 * The floppy DMA controller cannot cross a 64 KB physical boundary in one
 * INT 13h call.  BSS buffers inside the kernel (loaded at 0x10000) easily
 * straddle the 0x20000 boundary and cause BIOS error 0x09.  This single
 * sector-sized buffer sits entirely inside the 0x0000-0xFFFF page, so it
 * can never trigger the boundary condition.  Every sector is read/written
 * through it one at a time, then copied to/from the real destination.
 */
#define BOUNCE_PHYS 0x0800u
#define BOUNCE_PTR  ((uint8_t *)BOUNCE_PHYS)

/* Working buffers in BSS (used only as logical storage, never for DMA). */
...
/* ── BIOS thunk interface ─────────────────────────────────────────────── */
/*
 * bios_disk_thunk(drive, head, sector, cyl, count, buf_phys, use_edd, lba_lo)
 * Returns BIOS status byte (0 = OK).
 * Defined in entry.asm.
 */
extern uint8_t bios_disk_thunk(uint8_t  drive,
                                uint8_t  head,
                                uint8_t  sector,
                                uint8_t  cyl,
                                uint16_t count,
...
/* ── Low-level read (one sector at a time via bounce buffer) ──────────── */
static int disk_read(uint32_t lba, uint16_t count, uint32_t buf_phys)
{
    while (count) {
        ...
        /* Read exactly one sector into the bounce buffer. */
        uint8_t st = bios_disk_thunk(g_drive, head, sec, cyl,
                                     1, BOUNCE_PHYS, 0, 0);
        if (st) return (int)st;

        /* Copy from bounce to destination. */
...
/* ── Low-level write (one sector at a time via bounce buffer) ─────────── */
static int disk_write(uint32_t lba, uint16_t count, uint32_t buf_phys)
{
    while (count) {
        ...
        /* Copy from source into bounce buffer. */
        uint8_t *src = (uint8_t *)(uintptr_t)buf_phys;
        uint8_t *dst = BOUNCE_PTR;
        for (int i = 0; i < SECTOR_SZ; i++) dst[i] = src[i];

        /* Write one sector from bounce buffer. */
        uint8_t st = bios_disk_thunk(g_drive, head, sec, cyl,
                                     1, BOUNCE_PHYS, 2, 0);
        if (st) return (int)st;
...
/* Total data clusters on a 1.44 MB floppy */
...
static uint8_t *root_find(const char *name11)
{
    ...
    for (int i = 0; i < ROOT_ENTRIES; i++, p += 32) {
        ...
        if (p[11] & 0x08) continue; /* skip volume label */
        int match = 1;
        for (int j = 0; j < 11; j++)
...
int fat_save(const char *name11, const void *buf, int len)
{
    ...
    /* free existing cluster chain if overwriting */
    uint8_t *e = root_find(name11);
    if (e) {
    ...
    while (left > 0) {
        ...
        if (!c) return -1;
        fat12_set(c, 0xFF8); /* tentative EOF */
        ...
        if (take < SECTOR_SZ) {
            ...
            if (disk_write(clus_to_lba(c), 1,
                           (uint32_t)(uintptr_t)io_buf)) return -1;
        } else {
            if (disk_write(clus_to_lba(c), 1,
                           (uint32_t)(uintptr_t)src)) return -1;
        }
