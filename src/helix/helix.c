...
/* DEBUG: prints an unsigned byte as 2 hex digits. Used throughout
 * kernel_main() below to report the boot drive number and any status
 * codes exactly as received -- never summarized or guessed at. */
...
/* ── Kernel panic ─────────────────────────────────────────────────── */
void kpanic(const char *msg){term_set_color(VGA_RED,VGA_WHITE);term_puts(" HELIX FAILIURE ");term_set_color(VGA_YELLOW,VGA_RED);term_putchar(' ');term_puts(msg);term_putchar('\n');term_set_color(VGA_WHITE,VGA_RED);term_puts(" System halted. Power off or reset.\n");__asm__ __volatile__("cli");for(;;)__asm__ __volatile__("hlt");}
...
void draw_banner(void){term_clear();banner_row("",VGA_BLACK,VGA_CYAN);banner_row("BOREALIS",VGA_WHITE,VGA_BLUE);banner_row("x86-32  ver. 1.7.122-3r  29-07-2026",VGA_LIGHT_GREY,VGA_BLUE);banner_row("",VGA_BLACK,VGA_CYAN);term_set_color(VGA_DARK_GREY,VGA_BLACK);for(int i=0;i<COLS;i++)term_putchar('-');term_putchar('\n');term_set_color(VGA_LIGHT_GREY,VGA_BLACK);}

/* ── Kernel entry ─────────────────────────────────────────────────── */
/* DEBUG POLICY (permanent, not to be removed):
 * This kernel is meant to run on unknown/untested hardware with no
 * debugger and no serial port available in the general case. Every
 * step of boot that can fail MUST print an explicit, human-readable
 * checkpoint -- both on success and on failure -- so that whoever is
 * looking at the screen after a hang or crash knows EXACTLY which
 * subsystem was responsible, without needing to re-instrument
 * anything or guess. All such lines are tagged "[DEBUG ...]" and are
 * printed to the normal scrolling terminal (not a temporary line
 * that gets cleared), so they stay on screen and scroll into
 * history exactly like everything else BOREALIS prints. This is
 * intentional: a kernel like this should always be self-diagnosing. */
void kernel_main(uint8_t boot_drive){
    /* DEBUG: absolute first checkpoint inside helix.c. If you see
     * entry.asm's checkpoints (E Z T G ]) but never this line, the
     * call into kernel_main() itself is failing (stack corruption,
     * bad calling convention, or a linker/relocation problem). */
    term_set_color(VGA_LIGHT_GREEN, VGA_BLACK);
    term_puts("[DEBUG helix: kernel_main ENTER boot_drive=0x");
    dbg_hex8(boot_drive);
    term_puts("]\n");
    term_set_color(VGA_LIGHT_GREY, VGA_BLACK);

    /* DEBUG: flushing any stale PS/2 controller output buffer bytes
     * left over from the BIOS before we start reading keystrokes for
     * real. This loop is bounded so a stuck/faulty PS/2 controller
     * can never hang boot here silently -- if it maxes out, we say
     * so explicitly instead of just freezing forever. */
    term_puts("[DEBUG helix: flushing PS/2 keyboard buffer]\n");
    {
        int flushed = 0;
        int safety = 0;
        while(inb(0x64)&1){
            ...
            if(safety > 1000){
                ...
                term_puts("[DEBUG helix: WARNING keyboard buffer flush hit safety limit (possible stuck PS/2 controller) -- continuing anyway]\n");
                term_set_color(VGA_LIGHT_GREY, VGA_BLACK);
                break;
            }
        }
        term_puts("[DEBUG helix: keyboard buffer flush done, bytes_discarded=");
        term_puti(flushed);
        term_puts("]\n");
    }

    term_puts("[DEBUG helix: drawing boot banner]\n");
    draw_banner();
    /* NOTE: draw_banner() calls term_clear(), which wipes the two
     * debug lines printed just above. That is fine -- they already
     * did their job (proving we got this far) and everything from
     * here on stays on screen permanently, per the debug policy
     * above. */
    term_set_color(VGA_LIGHT_GREEN, VGA_BLACK);
    term_puts("[DEBUG helix: boot banner drawn ok]\n");
    term_set_color(VGA_LIGHT_GREY, VGA_BLACK);

    term_puts("[DEBUG helix: calling fat_init() -- see fs/fat12.c debug output below]\n");
    fat_init(boot_drive);
    ...
    if(!fat_ready()){
        ...
        term_set_color(VGA_LIGHT_GREEN, VGA_BLACK);
        term_puts("[DEBUG helix: filesystem ready, LOAD/SAVE/DIR available]\n");
        term_set_color(VGA_LIGHT_GREY, VGA_BLACK);
    }

    term_set_color(VGA_LIGHT_GREEN, VGA_BLACK);
    term_puts("[DEBUG helix: system checks complete -- starting BASIC interpreter]\n");
    term_set_color(VGA_LIGHT_GREY, VGA_BLACK);

    basic_run();

    /* DEBUG: basic_run() is documented to never return -- it either
     * loops forever servicing the REPL or the machine is reset. If
     * we ever land here, something inside basic.c fell through every
     * one of its own loops, which is a serious bug worth a very
     * loud, unmissable message rather than just falling into
     * kpanic()'s generic text. */
    term_set_color(VGA_LIGHT_RED, VGA_BLACK);
    term_puts("[DEBUG helix: FATAL basic_run() returned control to kernel_main() -- this should be impossible]\n");
    term_set_color(VGA_LIGHT_GREY, VGA_BLACK);
