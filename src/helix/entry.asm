...
; ── DEBUG: raw VGA text output, usable before helix.c's terminal is
; up. Writes to the BOTTOM row (row 24) so it never collides with the
; boot banner drawn later by helix.c. Column position is tracked in
; dbg_col (bss) and is NEVER reset, so every checkpoint from power-on
; onward appears left-to-right in order. This output is permanent and
; intentional -- do not strip it, it is the only visibility we have
; into the pre-C, pre-terminal part of boot.
DBG_VGA_ROW  equ 24
...
; DEBUG: prints '<' before dropping to real mode and '>' + result digit
