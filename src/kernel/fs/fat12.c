...
static uint8_t g_drive, g_use_edd = 0;
...
static int disk_read(uint32_t lba, uint16_t count, uint32_t buf_phys){
    while(count){
        ...
        uint8_t st=bios_disk_thunk(g_drive,head,sec,cyl,1,BOUNCE_PHYS,g_use_edd?1:0,lba);
        if(st) return (int)st;
        uint8_t *dst=(uint8_t*)(uintptr_t)buf_phys, *src=BOUNCE_PTR;
...
static int disk_write(uint32_t lba, uint16_t count, uint32_t buf_phys){
    while(count){
        uint8_t cyl=0,head=0,sec=0;
        if(!g_use_edd) lba_to_chs(lba,&cyl,&head,&sec);
        uint8_t *src=(uint8_t*)(uintptr_t)buf_phys, *dst=BOUNCE_PTR;
        for(int i=0;i<SECTOR_SZ;i++) dst[i]=src[i];
        uint8_t st=bios_disk_thunk(g_drive,head,sec,cyl,1,BOUNCE_PHYS,g_use_edd?3:2,lba);
        if(st) return (int)st;
        lba++;count--;buf_phys+=SECTOR_SZ;
    }
    return 0;
...
static void fat12_set(uint16_t clus,uint16_t val){
    ...
    if(clus&1) *p=(*p&0x000F)|(uint16_t)(val<<4);
    else        *p=(*p&0xF000)|(val&0x0FFF);
}
static uint32_t clus_to_lba(uint16_t clus){return DATA_LBA+(uint32_t)(clus-2)*SECS_PER_CLUS;}
#define TOTAL_CLUS ((uint16_t)((2880-DATA_LBA)/SECS_PER_CLUS+2))
static uint16_t fat12_alloc(void){
    for(uint16_t c=2;c<TOTAL_CLUS;c++) if(!fat12_get(c)) return c;
    return 0;
}
static int fat_flush(void){
    uint32_t phys=(uint32_t)(uintptr_t)fat_buf; int r=0;
    r|=disk_write(FAT1_LBA,FAT_SECS,phys);
    r|=disk_write(FAT2_LBA,FAT_SECS,phys);
    return r?-1:0;
}
static int root_flush(void){
    return disk_write(ROOT_LBA,ROOT_SECS,(uint32_t)(uintptr_t)root_buf)?-1:0;
}
static uint8_t *root_find(const char *name11){
    ...
    for(int i=0;i<ROOT_ENTRIES;i++,p+=32){
        ...
        int match=1;
        for(int j=0;j<11;j++) if(p[j]!=(uint8_t)name11[j]){match=0;break;}
        if(match) return p;
    }
    return 0;
}
static uint8_t *root_alloc_slot(void){
    uint8_t *p=root_buf;
    for(int i=0;i<ROOT_ENTRIES;i++,p+=32) if(!p[0]||p[0]==0xE5) return p;
    return 0;
}
void fat_init(uint8_t drive){
    g_drive=drive; fat_loaded=0; g_use_edd=0;
    if(drive>=0x80){uint8_t r=bios_disk_thunk(drive,0,0,0,0,0,4,0); if(!r) g_use_edd=1;}
...
int fat_save(const char *name11,const void *buf,int len){
    ...
    if(e){
        ...
        while(c>=2&&c<0xFF8){uint16_t nx=fat12_get(c);fat12_set(c,0);c=nx;}
    } else {
        e=root_alloc_slot(); if(!e) return -1;
        for(int i=0;i<32;i++) e[i]=0;
        for(int i=0;i<11;i++) e[i]=(uint8_t)name11[i];
        e[11]=0x20;
    }
    const uint8_t *src=(const uint8_t*)buf; int left=len;
    uint16_t first=0,prev=0;
    while(left>0){
        uint16_t c=fat12_alloc(); if(!c) return -1;
        fat12_set(c,0xFF8);
        if(prev) fat12_set(prev,c); else first=c;
        prev=c;
        int take=(left<SECTOR_SZ)?left:SECTOR_SZ;
        if(take<SECTOR_SZ){
...
void fat_dir(void(*cb)(const char*,uint32_t)){
    if(!fat_loaded) return;
    uint8_t *p=root_buf; char nm[12];
    for(int i=0;i<ROOT_ENTRIES;i++,p+=32){
        if(!p[0]||p[0]==0xE5) continue;
        if(p[11]&0x08) continue;
